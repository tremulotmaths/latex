...
\usepackage{xargs}%commande a plusieur option
...
\setlength{\columnseprule}{0mm} % enlève la barre multicol
\usepackage{fourier-orns} %% symboles notamment danger
\newcommand{\Card}[1]{\text{card}\left(#1\righ)}
\usepackage{tcolorbox}
%% Corrigé préremplie 

\def\corrige{oui} %% oui par défaut

\newcommand{\reponse}[1]{\ifthenelse{\equal{\corrige}{oui}}{#1}{{\color{White} #1 }}} % transforme le texte en blanc

\newcommand{\reponsegraph}[1]{\ifthenelse{\equal{\corrige}{oui}}{#1}{}}
% A utiliser dans les graphiques TIKZ

\raggedright %enlève la justification du texte
...
%Simplification de la notation du determinant \determinant{}

\newcommand{\determinant}[2]{\text{det}\left(\vect{#1},\vect{#2}\right)}
...
\renewcommand{\arraystretch}{1}%augmente la hauteur des lignes des tableaux
...
\newcolumntype{R}[1]{>{\raggedright\arraybackslash}m{#1}}%cellule alignée à droite et centrée verticalement
...
\definecolor{theocolor}{HTML}{763E77}%{HTML}{036DD0}%F4404D%rouge
...
\definecolor{excolor}{HTML}{C35A22}\definecolor{notacolor}{HTML}{75A43E}
\definecolor{demo@color}{HTML}{12891A}
\definecolor{methocolor}{HTML}{B03A2E}
...
\def\Pcolor{5} %Définition du pourcentage de couleur du fond des énoncés
%___________________________
...
\NewEnviron{Prop}[2][]{
\refstepcounter{prop}
\begin{tcolorbox}[colback=theocolor!\Pcolor!white,colframe=theocolor,colbacktitle=theocolor,title=\textbf{Propriété#1~\theprop~:~#2}]
\raggedright
\BODY
\end{tcolorbox}
}
...
%
\newcounter{notation}
\NewEnviron{Notation}[1][]{
\refstepcounter{notation}
\begin{tcolorbox}[colback=notacolor!\Pcolor!white,colframe=notacolor, colbacktitle=notacolor,title=\textbf{Notation#1~:}]
\raggedright
\BODY
\end{tcolorbox}
\end{tikzpicture}
...
\NewEnviron{Cons}[2][]{
\refstepcounter{cons}
\begin{tcolorbox}[colback=theocolor!\Pcolor!white,colframe=theocolor,title=\textbf{Conséquence#1~\thecons~:~#2}]
\raggedright
\BODY
\end{tcolorbox}
}
...
\begin{tcolorbox}[colback=notacolor!\Pcolor!white,colframe=notacolor,colbacktitle=notacolor,title=\textbf{Vocabulaire#1~\thevocab~:}]
\raggedright
\BODY
\end{tcolorbox}
}
...
\NewEnviron{Defi}[2][]{
\refstepcounter{defi}
\begin{tcolorbox}[colback=theocolor!\Pcolor!white,colframe=theocolor,colbacktitle=theocolor,title=\textbf{Définition#1~\thedefi~:~#2}]
\raggedright
\BODY
\end{tcolorbox}
}
...
\begin{tcolorbox}[colback=methocolor!\Pcolor!white,colframe=methocolor,colbacktitle=methocolor,title=\textbf{Méthode#1~\themethode~:~#2}]
\raggedright
\BODY
\end{tcolorbox}
}
...
\smallskip
\textcolor{subsection@title@color}{\textbf{Exemple#1~:}}\par
...
\NewEnviron{Demontration}{
\smallskip
\textcolor{demo@color}{\textbf{Démonstration~:}}\par
\begin{adjustwidth}{2em}{0em}
\raggedright
\BODY
\end{adjustwidth}
...
\NewEnviron{Application}[1][]{
\smallskip
\textbf{Application#1~:}\par
\begin{adjustwidth}{2em}{0em}
\raggedright
\BODY
\end{adjustwidth}
...
\newcounter{remarque}
\NewEnviron{Remarque}[1][]{
\smallskip
\textcolor{theocolor}{\textbf{\textit{Remarque#1~:}}}\par
\begin{adjustwidth}{2em}{0em}
\raggedright
\BODY
\end{adjustwidth}
...
\ifthenelse{\equal{\corrige}{oui}}{\BODY}{\textcolor{White}{\BODY}}
\smallskip
}
...
%
%\begin{center}
%\begin{}[somsuitar]{Calcul d'une somme}
%\begin{algovar}
...
colback=Red!5!white,
colframe=Red!85!black,fonttitle=\bfseries,
colbacktitle=Red!85!black,
...
\begin{mybox}[colback=blue!2]{#2} %Titre du cadre
\BODY
\end{mybox}
\end{minipage}
}
...
\begin{module}[colback=White,]{#1}%Catégorie du module
...
basicstyle=\ttfamilywithbold, % size of the fonts for the code
...
%% Rémi Ozgok 2 raccourci Tikz

%Définition de repère
\newcommandx{\Graphique}[8][5=1,6=1,7=2,8=2]{

\pgfmathsetmacro\resultx{1/#7}
\pgfmathsetmacro\resulty{1/#8}

	\ifthenelse{#1 = 0}{\def\margex{0}}{\def\margex{.5}}
	\ifthenelse{#2 = 0}{\def\margey{0}}{\def\margey{.5}}
	\draw [color=gray!30, xstep=\resultx ,ystep=\resulty] (-\margex +#1,-\margey +#2) grid (.5+#3,.5+#4);
	\draw[-stealth,line width=1pt,color=black] (-\margex + #1,0) -- (.5+#3,0);
	\draw[-stealth,line width=1pt,color=black] (0,-\margey +#2) -- (0,.5+#4);
	\foreach \x in {#1,...,#3}\draw[color=black] (\x,2pt) -- (\x,-2pt) node[below,font=\bfseries] {\ifthenelse{\not \x = 0}{\pgfmathparse{#5*\x}\pgfmathprintnumber{\pgfmathresult}}{}};
	\foreach \y in {#2,...,#4}\draw[color=black] (2pt,\y) -- (-2pt,\y) node[left,font=\bfseries] {\ifthenelse{\not \y = 0}{\pgfmathparse{#6*\y}\pgfmathprintnumber{\pgfmathresult}}{}};
	\draw[color=black] (-2pt,-2pt) node[below left,font=\bfseries] {\pgfmathparse{0*0}\pgfmathprintnumber{\pgfmathresult}};
\draw (0,#4+.25) node[right] {$y$};
\draw (#3+.25,0) node[above] {$x$};%%%%%%%%%%%marque 0 à l'origine, en bas, à gauche
\clip(#1-.5,#2-.5) rectangle (#3+.5,#4+.5);%%%%%%%%%%%%réduit la fenêtre d'affichage de ce qui suit
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%% Graphique sans numéro

\newcommandx{\GraphiqueS}[8][5=1,6=1,7=2,8=2]{
...
\pgfmathsetmacro\resulty{1/#8}
	...
	\ifthenelse{#1 = 0}{\def\margex{0}}{\def\margex{.5}}
	\ifthenelse{#2 = 0}{\def\margey{0}}{\def\margey{.5}}
	\draw [color=gray!10, xstep=\resultx ,ystep=\resulty, fill=White, rounded corners=0pt] (-\margex +#1,-\margey +#2) grid (.5+#3,.5+#4) rectangle (-\margex +#1,-\margey +#2);
	\draw[-stealth,line width=1pt,color=black] (-\margex + #1,0) -- (.5+#3,0);
	\draw[-stealth,line width=1pt,color=black] (0,-\margey +#2) -- (0,.5+#4);
	\draw[color=black] (-2pt,-2pt) node[below left,font=\bfseries] {\pgfmathparse{0*0}\pgfmathprintnumber{\pgfmathresult}};
\draw (0,#4+.25) node[right] {$y$};
...
%%%%%%%%%%%%%%%graphique repère Oij%%%%%%%%%%%%%%%%%%%%%
\newcommandx{\GraphiqueV}[4]{
	\ifthenelse{#1 = 0}{\def\margex{0}}{\def\margex{.5}}
	\ifthenelse{#2 = 0}{\def\margey{0}}{\def\margey{.5}}
	\draw [color=gray!20, fill=White, rounded corners=0pt] (-\margex +#1,-\margey +#2) grid (.5+#3,.5+#4) rectangle (-\margex +#1,-\margey +#2);
	\draw[-stealth,line width=1pt,color=black] (-\margex + #1,0) -- (.5+#3,0);
	\draw[-stealth,line width=1pt,color=black] (0,-\margey +#2) -- (0,.5+#4);
	\draw[color=black] (-2pt,-2pt) node[below left] {$O$};
\draw (0,#4+.25) node[right] {$y$};
\draw (#3+.25,0) node[above] {$x$};%%%%%%%%%%%marque 0 à l'origine, en bas, à gauche
\clip(#1-.5,#2-.5) rectangle (#3+.5,#4+.5);%%%%%%%%%%%%réduit la fenêtre d'affichage de ce qui suit
\Vecteur{0}{0}{1}{0}{\overrightarrow{\imath}}[Red][right]
\Vecteur{0}{0}{0}{1}{\overrightarrow{\jmath}}[Red][left]
...
%Droite gradué
\newcommandx{\Droite}[3][3=1]{
\ifthenelse{#1=0}{\def\depart{0}}{\def\depart{-.5 + #1}}

\draw[-stealth,line width=1pt,color=black] (\depart ,0)-- (#2+.5,0);
\foreach \x in {#1,...,#2}\draw[color=black] (\x,2pt) -- (\x,-2pt) node[below,font=\bfseries] {\pgfmathparse{#3*\x}\pgfmathprintnumber{\pgfmathresult}};

}

%%Vecteur[xo,yo,xu,yu, nom, couleur(defaut=Black) ,RL(defaut=right)]
\newcommandx{\Vecteur}[7][6=Black,7=right]{
\draw[very thick,>=latex,->,color = #6, auto=#7] (#1,#2)--(#1+#3,#2+#4) node[midway] {$#5$};}

%%Point[x,y,nom,position nom]
\newcommandx{\Point}[4][4=above]{
\draw (#1,#2) node[#4] {$#3$};
\draw (#1,#2) node[] {$\bm{+}$};
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
