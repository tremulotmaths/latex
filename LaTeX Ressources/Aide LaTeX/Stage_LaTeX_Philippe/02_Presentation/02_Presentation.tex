...
\setcounter{NumFiche}{1}

\entete{Stage \LaTeX}{Fiche \theNumFiche{} : Présentation du document}{}

%\makeindex
\begin{document}
\TitreFiche{Présentation du document}{\small Stage \LaTeX}
\thispagestyle{empty}
...
\begin{center}
    ...
    \hline
        \multirow{3}*{Familles} & romain (par défaut) & \verb!\textrm!\ArgObl{texte} & \verb*!\rmfamily !\Arg{texte} & \verb!rm! = roman\\
    \cline{2-5}
        & {\sffamily sans empattement} & \verb!\textsf!\ArgObl{texte} & \verb*!\sffamily !\Arg{texte} & \verb!sf! = sans serif \\
    \cline{2-5}
        & {\ttfamily à chasse fixe} & \verb!\texttt!\ArgObl{texte} & \verb*!\ttfamily !\Arg{texte} & \verb!tt! = teletype \\
    \hline\hline
        \multirow{4}*{Formes} & droit (par défaut) & \verb!\textup!\ArgObl{texte} & \verb*!\upshape !\Arg{texte} & \verb!up! = upright (droit)\\
      \cline{2-5}
        & \textsl{incliné} & \verb!\textsl!\ArgObl{texte} & \verb*!\slshape !\Arg{texte} & \verb!sl! = slanted (penché) \\
      \cline{2-5}
        & \textit{italique} & \verb!\textit!\ArgObl{texte} & \verb*!\itshape !\Arg{texte} & \verb!it! = italique \\
      \cline{2-5}
        & \textsc{petites capitales} & \verb!\textsc!\ArgObl{texte} & \verb*!\scshape !\Arg{texte} & \verb!sc! = small caps \\
    \hline\hline
        \multirow{2}*{Graisses} & médium (par défaut) & \verb!\textmd!\ArgObl{texte} & \verb*!\mdseries !\Arg{texte} & \verb!ms! = medium\\
      \cline{2-5}
        & \textbf{gras} & \verb!\textbf!\ArgObl{texte} & \verb*!\bfseries !\Arg{texte} & \verb!bf! = bold face (gras) \\
    \hline
    \end{tabular}
\end{center}
...
\begin{info}
    \NomCom{centering} est la commande semi-globale associé à l'environnement \ordi{center}. \NomCom{raggedleft} est associé à \ordi{flushright} et \NomCom{raggedright} est associée à \ordi{flushleft}.
\end{info}
...
Afin de colorer un document, on utilise le \package \NomPack{xcolor}, chargé dans le préambule à l'aide de la commande \verb!\usepackage{xcolor}!. \NomPack{xcolor} permet d'accéder aux couleurs suivantes :
\begin{center}
    \ttfamily
    \begin{tabular}{*{7}{l}}
...
\begin{info}
    ...
    La page 38 de la documentation du \package permet d'en savoir davantage. Il suffit de taper sur un moteur de recherche \ttfamily{LaTeX xcolor doc} pour obtenir ce que l'on cherche.
\end{info}
...
Le \package \NomPack{geometry} est utilisé pour régler la géométrie de la page indépendamment du choix de la \jargon{classe} : dimensions du papier, orientation (portrait, paysage), dimensions des marges, particularités d'un document recto-verso, dimensions des en-têtes et pieds-de-pages...
...
Pour écrire une partie d'un document sur deux ou plusieurs colonnes, on a recourt au \package \NomPack{multicol} qui nous permet alors d'accéder à l'\jargon{environnement} \ordi{multicols}.
...
Toutes ces listes peuvent s'imbriquées les unes dans les autres en mélangeant ou non les différents types. On utilisera avantageusement les tabulations pour une présentation claire du \jargon{code source}.\bigskip
...
\begin{info}
    ...
    De plus, il permet de reprendre la numérotation d'une liste \ordi{enumerate} qui a été interrompue. La lecture de la documentation de ce \package est vivement conseillée.
\end{info}
...
\begin{info}
    ...
    \NomCom{section*\{Introduction\}} à la place de \NomCom{section\{Introduction\}}.
\end{info}
